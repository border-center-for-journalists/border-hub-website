...
import { NoticeSection, NormalNoticeContainer, SocialContainer,SocialIcon } from "./index.styled"
...
class NormalNoticeComponent extends Component {
  ...
  handleScroll = () => {
    const isSticky = window.scrollY > 80; // 80 es la posición a partir de la cual el menú se vuelve pegajoso
    this.setState({ isSticky });
  }

  componentDidMount() {
    window.addEventListener("scroll", this.handleScroll); // Agregar el evento scroll
  }

  componentWillUnmount() {
    window.removeEventListener("scroll", this.handleScroll); // Remover el evento scroll
  }
  ...
  render() {
    const { isSticky } = this.state; // Obtener el valor actual de isSticky
    return (
      <NoticeSection color="black">
        ...
        <NormalNoticeContainer>
          ...
          <LinkedinShareButton url={this.props.url} title={this.props.notice.data.title.text}>
            <SocialIcon className="icon-linkedin iconSvg" >
          ...
          <WhatsappShareButton url={this.props.url} >
            <SocialIcon className="icon-whatsapp iconSvg" >
              <svg viewBox="0 0 64 64" width="30" height="30" >
                <g>
